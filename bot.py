        ...
        for line_number, line in enumerate(file, 1):
            if detect_unusual_text(line):
                ...
                if webhook_url != "" and use_log == True:
                    ...
                    embed.set_footer(text="BoostBot Crafted by Tools Cord")
...
class BoostModal(Modal):
    def __init__(self):
        super().__init__(title = "ToolsCord - Boost Panel")
        self.add_item(
            TextInput(
                label = "Invite",
    ...
    async def on_submit(self, ctx: discord.Interaction):
        ...
        await ctx.followup.send(
            ...
            ).set_footer(text="BoostBot Crafted by Tools Cord")
        )
...
@bot.tree.command(name="help", description="View all available Tools Cord commands")
...
class LivestockModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        if duration not in [1, 3]:
            return await interaction.followup.send(
                embed=discord.Embed(
                    ...
                    color=0x2F3136,
                ).set_footer(text="BoostBot Crafted by Tools Cord"), ephemeral=True
            )
        ...
        initial_embed.set_footer(text="BoostBot Crafted by Tools Cord")
...
class RestockModal(discord.ui.Modal, title="ToolsCord - Restock Panel"):

    box_text = discord.ui.TextInput(
        label="Write what you want to restock:",
        placeholder="Example: Nitro token etc...",
        style=discord.TextStyle.paragraph,  # large input box
        required=True,
        max_length=2000
    )
...
@bot.tree.command(
    name="restock3",
    description="Open a dialog to restock a box"
)
async def restock3(interaction: discord.Interaction):

    # Show modal when user triggers the slash command
    await interaction.response.send_modal(RestockModal())

@bot.tree.command(
    name="restock2",
    description="Open a box and write what you want into data/1m.txt"
)
@app_commands.describe(
    text="Text that will be added to the box file"
)
async def restock2(interaction: discord.Interaction, text: str):
    ...
    async def on_submit(self, ctx: discord.Interaction):   
        ...
        try:
            ...
            if key_info:
                ...
                embed.set_footer(text="Crafted With <3 By Ð00M")
...
class Auto_Boost_Modal(Modal):
    def __init__(self):
        super().__init__(title = "Tools Cord BoostBot")
        self.add_item(
            TextInput(
                label = "Key",
                placeholder = "Enter your key.",
    ...
    try:
        ...
        await ctx.response.send_message(
            embed=discord.Embed(
                ...
                color=0xff00bb
            ).set_footer(text="Crafted With <3 By Ð00M"),
            ephemeral=True
        )
    except FileNotFoundError:
        await ctx.response.send_message(
